...
% FG MATLAB code for FG.fig
%      FG, by itself, creates a new FG or raises the existing
%      singleton*.
%
%      H = FG returns the handle to a new FG or the handle to
%      the existing singleton*.
%
%      FG('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in FG.M with the given input arguments.
%
%      FG('Property','Value',...) creates a new FG or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before FG_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to FG_OpeningFcn via varargin.
%
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @FG_OpeningFcn, ...
                   'gui_OutputFcn',  @FG_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before FG is made visible.
function FG_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to FG (see VARARGIN)

% Choose default command line output for FG
handles.output = hObject;
axes(handles.axes1); axis off
axes(handles.axes2); axis off
axes(handles.axes3); axis off
axes(handles.axes4); axis off
% axes(handles.axes8); axis off
...
set(handles.edit20,'String','Result Display');
set(handles.edit21,'String','Result Display');


% Update handles structure
guidata(hObject, handles);

% UIWAIT makes FG wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = FG_OutputFcn(hObject, eventdata, handles) 
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global a;global skw;global en;global k; 
[fname,path]=uigetfile('*.*','Browse Image');
if fname~=0
    a=imread([path,fname]);
    axes(handles.axes1); imshow(a); title('Original Image');
else
...
% axes(handles.axes8); cla(handles.axes8); title(''); axis off
